...
function adminPinOK(req, body = {}) {
  ...
  const expected = (process.env.IHS_TEST_ADMIN_PIN || "").trim();
...
function isPast(date, start) {
  return `${date} ${start}:00` <= shanghaiNow();
}
...
async function getStudentPayload(sql, student) {
  ...
  const out = { ...student, booking: active[0] ? { ...active[0], end: active[0].end } : null };
  ...
  out.slots = rows
    .filter(r => Array.isArray(r.eligible_classes) && r.eligible_classes.includes(student.class_name))
    .filter(r => !isPast(r.date, r.start))
    .map(r => ({
      slot_key: r.slot_key,
      date: r.date,
      start: r.start,
...
async function handle(req) {
  if (!process.env.IHS_TEST_ADMIN_PIN) {
    return json({ error: "Netlify 尚未设置 IHS_TEST_ADMIN_PIN 环境变量" }, 500);
  }
  ...
  if (path === "/api/health") {
    return json({ ok: true, mode: "netlify-internal-test", time: shanghaiNow() });
  }
  ...
  if (path === "/api/student/lookup" && req.method === "POST") {
    ...
    const rows = await sql`
      SELECT * FROM students WHERE UPPER(token) = ${token} LIMIT 1
    `;
    if (!rows[0]) return json({ error: "未找到该测试预约码，请核对后重试。" }, 404);
    return json({ student: await getStudentPayload(sql, rows[0]) });
  }
  ...
  if (path === "/api/book" && req.method === "POST") {
    ...
    try {
      const result = await sql.begin(async tx => {
        ...
        `;
        const s = students[0];
        if (!s) throw new Error("无效预约码");
        if (s.cas_key === "Coco") throw new Error("Coco 学生本轮另行预约");
        if (s.class_name === "G11-3") throw new Error("G11-3 班主任时间尚未导入，暂不开放");
        ...
        const old = await tx`
          ...
          LIMIT 1
        `;
        if (old[0]) throw new Error("该学生已经有一个有效预约，请先取消原预约");

        const slots = await tx`
          SELECT * FROM slots WHERE slot_key = ${slotKey} LIMIT 1
        `;
        const sl = slots[0];
        ...
        if (!sl.published || !sl.g12_locked) throw new Error("该时段尚未正式发布");
  ...
  if (path === "/api/cancel" && req.method === "POST") {
    ...
    try {
      const result = await sql.begin(async tx => {
        ...
        `;
        const b = bookings[0];
        if (!b) throw new Error("没有需要取消的预约");

        await tx`
          UPDATE bookings
          SET status = 'cancelled', cancelled_at = ${shanghaiNow()}
          WHERE id = ${b.id}
        `;
        await tx`
          UPDATE slots SET booked_student_id = NULL
          WHERE slot_key = ${b.slot_key} AND booked_student_id = ${s.id}
        `;
      ...
      return json({ error: e.message || "取消失败" }, 409);
    }
  }

  if (path === "/api/admin/state" && req.method === "GET") {
    if (!adminPinOK(req)) return json({ error: "管理员测试码不正确" }, 403);
    ...
    return json({ students, slots, bookings: bookings.map(b => ({ ...b, end: b.end })), stats });
  }
  ...
  if (path === "/api/admin/export.csv" && req.method === "GET") {
    ...
    const header = ["中文姓名","English Name","班级","班主任","升导","日期","开始","结束","预约时间"];
    ...
    return new Response("\uFEFF" + lines.join("\r\n"), {
      ...
      headers: {
        ...
        "content-disposition": 'attachment; filename="G11_test_bookings.csv"',
        "cache-control": "no-store"
      }
    });
  }
  ...
  if (path.startsWith("/api/admin/") && req.method === "POST") {
    const body = await req.json().catch(() => ({}));
    if (!adminPinOK(req, body)) return json({ error: "管理员测试码不正确" }, 403);
    ...
    if (path === "/api/admin/slot") {
      ...
      if (action === "lock_publish") {
        ...
        if (!sl.g12_locked) return json({ error: "请先模拟从G12端锁定" }, 409);
        await sql`UPDATE slots SET published = 1 WHERE slot_key = ${key}`;
      } else {
    ...
    if (path === "/api/admin/sample-publish") {
      const published = await sql.begin(async tx => {
        await tx`UPDATE slots SET g12_locked = 0, published = 0 WHERE booked_student_id IS NULL`;
        const demands = await tx`
          SELECT class_name, cas_key, COUNT(*)::int AS n
          FROM students
          WHERE round_status = '纳入本轮'
            AND cas_key <> 'Coco'
            AND class_name IN ('G11-1','G11-2')
          GROUP BY class_name, cas_key
          ORDER BY class_name, cas_key
        `;

        const selected = new Set();
        for (const d of demands) {
          const need = Math.min(2, d.n);
          const candidates = await tx`
            SELECT * FROM slots
            WHERE cas_key = ${d.cas_key}
            ORDER BY date, start
          `;
          const eligible = candidates
            .filter(r => Array.isArray(r.eligible_classes) && r.eligible_classes.includes(d.class_name))
            .filter(r => !isPast(r.date, r.start))
            .map(r => r.slot_key);

          let have = eligible.filter(k => selected.has(k)).length;
        ...
        for (const key of selected) {
          await tx`UPDATE slots SET g12_locked = 1, published = 1 WHERE slot_key = ${key}`;
        }
    ...
    if (path === "/api/admin/reset") {
      await sql.begin(async tx => {
        ...
        await tx`UPDATE slots SET booked_student_id = NULL, g12_locked = 0, published = 0`;
      });
      return json({ ok: true });
    }
  }
